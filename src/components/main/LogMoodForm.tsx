...
export default function LogMoodForm({ onSubmit }: { onSubmit: (moodData: any) => void } ) {
  ...
  return (
    <div className='p-8 pt-2'>
        <div className='flex justify-between items-start flex-col gap-8'>
            <h2 className='text-4xl font-bold'>Log your mood</h2>
            <div className='w-full'>
                <StepProgress currentStep={currentStep} />
            </div>
            {currentStep === 1 && <OverallModeStep overallMood={overallMood} handleOverallMoodChange={handleOverallMoodChange} />}
            {currentStep === 2 && <FeelingStep feelings={feelings} handleFeelingsChange={handleFeelingsChange} />}
            {currentStep === 3 && <JournalEntry journalEntry={journalEntry} handleJournalEntryChange={handleJournalEntryChange} />}
            {currentStep === 4 && <TotalHoursStep sleepHours={sleepHours} handleSleepHoursChange={handleSleepHoursChange} />}
            <div className='w-full flex justify-between items-center gap-4'>
            {currentStep <= 3 && <button onClick={handleNextStep} className='bg-indigo-500 text-white px-4 py-4 rounded-md w-full text-lg font-semibold'>Continue</button>}
            {currentStep > 3 && <button onClick={() => onSubmit({ overallMood, feelings, journalEntry, sleepHours })} className='bg-indigo-500 text-white px-4 py-4 rounded-md w-full text-lg font-semibold'>Submit</button>}
