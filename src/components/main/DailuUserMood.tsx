    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div className="md:col-span-2 bg-white rounded-lg shadow-sm p-6 flex flex-col gap-10 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <h1 className="text-4xl font-bold">{moodMap[userModeData?.overallMood as keyof typeof moodMap]}</h1>
        </div>
        <div className="z-10 absolute bottom-6 left-6 hidden md:block">
          <i className="font-semibold">"When your heart is full, share your light with the world."</i>
        </div>
      </div>
      <div className="md:col-span-1 flex flex-col gap-4">
        <div className="flex flex-col gap-4 bg-white rounded-lg shadow-sm p-6">
          <h5 className="text-gray-500 font-semibold flex items-center gap-2"><img src={iconSleep} alt="Sleep" className="w-6 h-6" /> Sleep</h5>
          <h1 className="text-3xl font-bold">{userModeData?.sleepHours} hours</h1>
        </div>
        <div className="flex flex-col gap-2 bg-white rounded-lg shadow-sm p-6">
          <h5 className="text-gray-500 font-semibold flex items-center gap-2"><img src={iconReflection} alt="Reflection" className="w-6 h-6" /> Reflection of the day</h5>
          <div className="flex flex-col gap-12">
            <p className="first-letter:capitalize font-semibold">{userModeData?.journalEntry}</p>
            <i className="text-gray-500 font-semibold">{userModeData?.feelings.map((feeling: string) => `#${feeling.charAt(0).toUpperCase() + feeling.slice(1)}`).join(' ')}</i>
          </div>
